...
A single install script that sets up a fast, opinionated shell environment on any Linux or macOS machine. Run it on a fresh box and get a full terminal workflow — persistent sessions, fuzzy search, smart directory jumping, a rich git-aware prompt, and history suggestions — in minutes.
...
Completion options with descriptions appear below the prompt as you type; `↓` enters the grid for any command, not just `cd`. `↑` searches older commands matching what you've typed so far; `↓` then moves forward through matches. `..` goes up a directory. `z` jumps anywhere by keyword.
...
`sshv` wraps `ssh` with sensible defaults for flaky networks:

- **15-second connect timeout** and client keepalives (`ServerAliveInterval=10`, `ServerAliveCountMax=2`) — skipped if you pass your own `ServerAliveInterval`.
- **Terminal cleanup on exit** — resets local input modes so mouse/keyboard modes from remote tmux/Zellij/vim do not leak into your shell after disconnect.
- **Automatic retry** — in interactive terminals, if the session exits with code `255` and the connection was actually established before dropping, `sshv` retries once. Failures where the host was never reached (timeout, wrong hostname, VPN down) are not retried; you get a short hint instead.

```bash
sshv user@host
```

Normal `ssh` is unchanged.
...
The following features are off by default. Each one requires a small config step described in [SETUP_DETAILS.md](SETUP_DETAILS.md).
...
Requires an OpenVPN `.ovpn` config file and your credentials. The installer sets up the helper scripts; you fill in the credentials file it creates.
...
Hosts in `~/.ssh/config` are tab-completed. Requires `sshfs` (installed by `setup_zsh.sh`; macOS also needs the `macfuse` cask).
